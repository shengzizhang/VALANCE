...
args <- commandArgs(trailingOnly = TRUE) #This returns only the arguments after the script name.
...
localH2O <- h2o.init( max_mem_size = paste( memory,
		      ...
		      ip = "localhost", port = 54774)


# Check if H2O cluster is running
if (!h2o.clusterIsUp()) {
  stop("H2O cluster is not up. Please check the H2O initialization parameters.")
}
