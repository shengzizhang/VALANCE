...
def catnap_by_antibodies(): 
    ...
    for (antibody, virus), df in tqdm(assays_df.groupby(['Antibody', 'Virus'])):
        ...
        if is_sensitive.sum() == len(is_sensitive):
            outcome = True
            # estimated_censored_mean=10000000000
        ...
        # estimate the mean taking into account the censored data
            ...
            outcome = estimated_censored_mean < IC50_TRESHOLD
        # else:
        #     outcome = 0
        #     estimated_censored_mean=200000000000000
        assays[antibody][virus] = outcome
        # assays[antibody][virus] = (outcome, estimated_censored_mean)
    return assays
...
def flatten_catnap_data(catnap_data):
    ...
    return flat
    # for antibody, viruses in catnap_data.items():
    #     for virus, (outcome, est_mean) in viruses.items():
    #         flat.append((id, antibody, virus, outcome, est_mean))
    #         id += 1
    # return flat
